...
export function RenderedSpawn({ spawn, fallback, maxDisplay }) {
  const model = useMemo(() => raceData.find(r => r.id === spawn.race)[spawn.gender] || 'HUM', [spawn]);
  const spawnRef = useRef();
  let scene, animations;
  try {
    ({scene, animations} = useLoader(GLTFLoader, `${storageUrl}/${model.toLowerCase()}.glb`)); // eslint-disable-line
  } catch (e) {
