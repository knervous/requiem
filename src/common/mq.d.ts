...
type SpawnListener = (spawn: MqSpawn) => void
...
declare function targetDied(spawn: MqSpawn): Promise<ConditionResult>
...
interface Listeners {
  // Spawn
  addSpawnListener(listener: SpawnListener): void
  removeSpawnListener(listener: SpawnListener): void

  // Despawn
  addDespawnListener(listener: SpawnListener): void
  removeDespawnListener(listener: SpawnListener): void

  // Zone
  addZoneListener(listener: ZoneListener): void
  removeZoneListener(listener: ZoneListener): void

  // Chat
  addChatListener(listener: ChatListener): void
  removeChatListener(listener: ChatListener): void
}
...
interface Util {
   ...
   * @param id Spawn ID
   */
  getSpawnById(id: string): ?MqSpawn

  /**
   * @returns An array of spawns
   */
  getZoneSpawns(): Array<MqSpawn>
   ...
   */
  getNthNearestSpawn(exactName: string, n: number = 1): MqSpawn

  /**
   *
   * @param spawnNames Array of exact names to match
   */
  getClosestOfSpawnNames(spawnNames: Array<string>): ?MqSpawn
   ...
   */
  distanceBetweenSpawns(spawn1: MqSpawn, spawn2: MqSpawn): number
   ...
   */
  main(fn: Function, loopSpeed: number): Promise

  /**
   *
   * @param port Port to listen for debugger. Default is 9229
   */
  useDebugger(port: number): void
}

interface MqConditions {
  /**
   * @param  {...Condition} conditions
   * @summary Wait for any of the parameters to finish no matter the order
   */
  waitForAny(...conditions: Array<Conditions>): ConditionResult

  /**
   * @param  {...Condition} conditions
   * @summary Wait for all of the parameters to finish
   */
  waitForAll(...conditions: Array<Conditions>): ConditionResult

  /**
   * @param {ConditionResult} result
   * Description of the condition result
   */
  getConditionResult(result): string
}

/** Object Interfaces **/
declare interface MQGlobal {
  Enums: {
    Condition: {
      [P in keyof typeof Conditions]: Condition
    }
    ConditionResult: {
      [P in keyof typeof ConditionResult]: ConditionResult
    }
  }
  /**
